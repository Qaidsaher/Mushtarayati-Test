...
class DayServices {
  ...
  static Future<File> createDayExcel(DateTime date, {String totalKey = 'total'}) async {
  ...
  final dateStr = '${date.year.toString().padLeft(4, '0')}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
    ...
    for (final m in menus) {
      ...
      // headers
      final headers = ['رقم', 'ملاحظة', 'كمية', 'سعر الوحدة', 'الفئة', 'الإجمالي'];
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        menuTotal += lineTotal;
        sheet.appendRow([i + 1, it.notes ?? '', it.qty, it.unitPrice, cat?.name ?? '', lineTotal]);
      }

      // totals row for this menu
      sheet.appendRow(['', '', '', '', 'الإجمالي', menuTotal.toStringAsFixed(2)]);
      // empty row as spacer
      sheet.appendRow([]);
    }
    ...
    final outName = 'day_services_${date.year.toString()}${date.month.toString().padLeft(2, '0')}${date.day.toString().padLeft(2, '0')}';
    final file = File('${dir.path}/${outName}_${DateTime.now().millisecondsSinceEpoch}.xlsx');
  ...
  static Future<File> createDayPdf(DateTime date, {String fontAsset = 'assets/fonts/NotoNaskhArabic-Regular.ttf', String logoAsset = 'assets/images/logo.png', String totalKey = 'الإجمالي'}) async {
    final menuRepo = MenuRepository();
    final itemRepo = ItemRepository();
    final catRepo = CategoryRepository();
  ...
  final dateStr = '${date.year.toString().padLeft(4, '0')}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
    ...
    for (var p = 0; p < allSections.length; p += chunkSize) {
      final chunk = allSections.sublist(p, (p + chunkSize) > allSections.length ? allSections.length : p + chunkSize);
      doc.addPage(pw.MultiPage(
        ...
        build: (context) {
          ...
          widgets.add(pw.Directionality(
            ...
            child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
              pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children: [
                pw.Text('تقرير اليوم: $dateStr', style: pw.TextStyle(font: arabicFont, fontSize: 16, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 6),
                pw.Text('عدد القوائم في الصفحة: ${chunk.length}', style: pw.TextStyle(font: arabicFont, fontSize: 10)),
              ])),
          ...
          for (final section in chunk) {
            ...
            widgets.add(pw.Container(padding: const pw.EdgeInsets.symmetric(vertical: 6), child: pw.Text(menuName, style: pw.TextStyle(font: arabicFont, fontSize: 13, fontWeight: pw.FontWeight.bold))));
            ...
            for (var i = 0; i < rows.length; i++) {
              ...
              double parsed = 0.0;
              if (candidate != null) {
                if (candidate is num) parsed = candidate.toDouble();
                else parsed = double.tryParse(candidate.toString().replaceAll(',', '.')) ?? 0.0;
              }
              totalSum += parsed;
              rowValues.add('${i + 1}');
            ...
            widgets.add(pw.Directionality(textDirection: pw.TextDirection.rtl, child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('عدد العناصر: ${rows.length}', style: pw.TextStyle(font: arabicFont, fontSize: 11, fontWeight: pw.FontWeight.bold)), pw.Text('الإجمالي: ${totalSum.toStringAsFixed(2)}', style: pw.TextStyle(font: arabicFont, fontSize: 11, fontWeight: pw.FontWeight.bold))])));
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/day_services_${date.year.toString()}${date.month.toString().padLeft(2, '0')}${date.day.toString().padLeft(2, '0')}.pdf');
